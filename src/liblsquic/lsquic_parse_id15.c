...
/*
 * lsquic_parse_id15.c -- Parsing functions specific to Internet Draft 12
...
static uint64_t
id15_read_float_time16 (const void *mem)
{
...
static void
id15_write_float_time16 (lsquic_time_t time_us, void *mem)
{
    assert(0);  /* Not implemented for ID-11 yet */
    uint16_t ret = 0;
    uint16_t high, i;
...
static void
id15_parse_packet_in_finish (lsquic_packet_in_t *packet_in,
...
static size_t
id15_packout_header_size_short (const struct lsquic_conn *lconn,
...
static int
id15_gen_reg_pkt_header (const struct lsquic_conn *lconn,
            const struct lsquic_packet_out *packet_out, unsigned char *buf,
                                                                size_t bufsz)
{
...
static void
id15_packno_info (const struct lsquic_conn *lconn,
...
static size_t
id15_packout_size (const struct lsquic_conn *lconn,
...
static int
id15_gen_stream_frame (unsigned char *buf, size_t buf_len,
...
static int
id15_gen_crypto_frame (unsigned char *buf, size_t buf_len,
...
static int
id15_parse_stream_frame (const unsigned char *buf, size_t rem_packet_sz,
...
static int
id15_parse_crypto_frame (const unsigned char *buf, size_t rem_packet_sz,
                                        struct stream_frame *stream_frame)
{
    const unsigned char *const pend = buf + rem_packet_sz;
    const unsigned char *p = buf;
...
static int
id15_parse_ack_frame (const unsigned char *const buf, size_t buf_len,
                                            struct ack_info *ack, uint8_t exp)
{
    ...
    if (0x1B == buf[0])
    {
        /* Three ECN counters follow, which we ignore */
        for (i = 0; p < end && i < 3; ++i)
            p += 1 << (*p >> 6);
        if (i < 3 || p > end)
            return -1;
...
static int
id15_gen_stop_waiting_frame(unsigned char *buf, size_t buf_len,
...
static int
id15_parse_stop_waiting_frame (const unsigned char *buf, size_t buf_len,
                 lsquic_packno_t cur_packno, enum lsquic_packno_bits bits,
...
static int
id15_skip_stop_waiting_frame (size_t buf_len, enum lsquic_packno_bits bits)
{
    assert(0);  /* Not implemented for ID-11 yet */
    unsigned packnum_len = packno_bits2len(bits);
    if (buf_len >= 1 + packnum_len)
...
static int
id15_gen_window_update_frame (unsigned char *buf, int buf_len,
...
static int
id15_parse_window_update_frame (const unsigned char *buf, size_t buf_len,
...
static int
id15_gen_blocked_frame (unsigned char *buf, size_t buf_len,
...
static int
id15_gen_rst_frame (unsigned char *buf, size_t buf_len,
...
static int
id15_parse_rst_frame (const unsigned char *buf, size_t buf_len,
...
static int
id15_parse_connect_close_frame (const unsigned char *buf, size_t buf_len,
...
static int
id15_gen_goaway_frame (unsigned char *buf, size_t buf_len,
...
static int
id15_parse_goaway_frame (const unsigned char *buf, size_t buf_len,
...
static int
id15_gen_ack_frame (unsigned char *outbuf, size_t outbuf_sz,
...
#define CHECKOUT(sz) do {                                               \
    ...
    *p = 0x1A;
    ...
    while ((range = rechist_next(rechist)))
    {
        ...
        if (!shifted && addl_ack_blocks == VINT_MAX_ONE_BYTE)
        {
            (void) block_count_p;
...
static size_t
id15_calc_stream_frame_header_sz (lsquic_stream_id_t stream_id,
...
static size_t
id15_calc_crypto_frame_header_sz (uint64_t offset)
{
    return 1    /* Frame type */
         + (1 << vint_val2bits(offset))
         + 1    /* Data len */
...
id15_parse_frame_type (unsigned char byte)
{
...
void
id15_turn_on_fin (unsigned char *stream_frame_header)
{
...
id15_calc_packno_bits (lsquic_packno_t packno,
...
const struct parse_funcs lsquic_parse_funcs_id15 =
{
    ...
    .pf_parse_packet_in_finish        =  id15_parse_packet_in_finish,
    .pf_gen_stream_frame              =  id15_gen_stream_frame,
    .pf_calc_stream_frame_header_sz   =  id15_calc_stream_frame_header_sz,
    .pf_parse_stream_frame            =  id15_parse_stream_frame,
    .pf_parse_ack_frame               =  id15_parse_ack_frame,
    .pf_gen_ack_frame                 =  id15_gen_ack_frame,
    .pf_gen_stop_waiting_frame        =  id15_gen_stop_waiting_frame,
    .pf_parse_stop_waiting_frame      =  id15_parse_stop_waiting_frame,
    .pf_skip_stop_waiting_frame       =  id15_skip_stop_waiting_frame,
    .pf_gen_window_update_frame       =  id15_gen_window_update_frame,
    .pf_parse_window_update_frame     =  id15_parse_window_update_frame,
    .pf_gen_blocked_frame             =  id15_gen_blocked_frame,
    .pf_parse_blocked_frame           =  id15_parse_blocked_frame,
    .pf_gen_rst_frame                 =  id15_gen_rst_frame,
...
#ifndef NDEBUG
    .pf_write_float_time16            =  id15_write_float_time16,
    .pf_read_float_time16             =  id15_read_float_time16,
#endif

                  ...
                  unsigned char *const buf, size_t bufsz)
{
    ...
    if (params->tp_idle_timeout != TP_DEF_IDLE_TIMEOUT)                 /* 3 */
    ...
    if (params->tp_max_ack_delay != TP_DEF_MAX_ACK_DELAY)              /* 12 */
        need += 4 + sizeof(params->tp_max_ack_delay);
...
#define WRITE_PARAM_TO_P(tpidx, tpval, width) do {                      \
    ...
    if (params->tp_idle_timeout != TP_DEF_IDLE_TIMEOUT)
...
#define EXPECT_LEN(expected_len) do {                               \
    ...
    while (p < end)
    {
        ...
        case TPI_MAX_ACK_DELAY:
            EXPECT_LEN(sizeof(params->tp_max_ack_delay));
            params->tp_max_ack_delay = p[0];
            break;
...
#define WRITE_ONE_PARAM(name, fmt) do {  \
    ...
    WRITE_ONE_PARAM(init_max_stream_data_bidi_local, "%"PRIu32);
    WRITE_ONE_PARAM(init_max_stream_data_bidi_remote, "%"PRIu32);
    WRITE_ONE_PARAM(init_max_stream_data_uni, "%"PRIu32);
    WRITE_ONE_PARAM(init_max_data, "%"PRIu32);
    WRITE_ONE_PARAM(idle_timeout, "%"PRIu16);
    WRITE_ONE_PARAM(init_max_bidi_streams, "%"PRIu16);
    WRITE_ONE_PARAM(init_max_uni_streams, "%"PRIu16);
    WRITE_ONE_PARAM(max_packet_size, "%"PRIu16);
    WRITE_ONE_PARAM(ack_delay_exponent, "%"PRIu8);

                ...
                unsigned initial_stream_window, const struct parse_funcs *pf)
{
    ...
    tobjs->lconn.cn_version = tobjs->lconn.cn_pf == &lsquic_parse_funcs_id18 ?
        LSQVER_ID18 : LSQVER_043;
...
test_hq_framing (int sched_immed, int dispatch_once, unsigned wsize)
{
    ...
    init_test_objs(&tobjs, buf_out_sz, buf_out_sz, &lsquic_parse_funcs_id18);
    tobjs.stream_if_ctx = &packet_stream_ctx;
...
main (int argc, char **argv)
{
    ...
    g_use_crypto_ctor = 1;
    g_pf = select_pf_by_ver(LSQVER_ID18);
    main_test_packetization();
